...
if ($Visible) {
    ...
    $backendProc = Start-Process -FilePath "cmd.exe" -ArgumentList "/k cd /d `"$PSScriptRoot`" && `"$pyExe`" backend\manage.py runserver 0.0.0.0:8001" -PassThru
    ...
    $mailProc = Start-Process -FilePath "cmd.exe" -ArgumentList "/k cd /d `"$PSScriptRoot`" && `"$pyExe`" tools\mail-server\dev_mail_server.py" -PassThru
} else {
    ...
    $mailLog = Join-Path $PSScriptRoot ".logs\mail.log"
    $mailErrLog = Join-Path $PSScriptRoot ".logs\mail.err.log"
